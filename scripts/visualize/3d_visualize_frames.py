...
for file_name in [sorted(list(os.listdir(directory)))[0]]:
    ...
    # plt.imshow(datas["Calibrated Distance"])
    plt.scatter(datas["Calibrated xVector"], datas["Calibrated yVector"], datas["Calibrated Distance"])

    print("x_v = ", datas["Calibrated xVector"].to_numpy().tolist())
    print("y_v = ", datas["Calibrated yVector"].to_numpy().tolist())
    print("d_v = ", datas["Calibrated Distance"].to_numpy().tolist())
    plt.title(directory + file_name)
    plt.show()

    time.sleep(0.1)

    # X = pd.read_csv(directory + "d1_0001.dat", delimiter="\t")
